...
app.get("/filter", (req, res) => {
  ...
  jokes.forEach((joke) => {
    if (joke.jokeType === jokeType) queriedJokes.push(joke);
  });

  if (queriedJokes.length > 0) res.send(queriedJokes);
  else res.status(404).send("No jokes for the said type.");
});
...
app.put("/jokes/:id", (req, res) => {
  let id = parseInt(req.params.id);
  let editedJoke = jokes.find((joke) => joke.id === id);
  editedJoke.jokeText = req.body.text;
  editedJoke.jokeType = req.body.type;

  if (editedJoke) res.send(editedJoke);
  else res.status(500).send("Internal Server Error");
});
...
app.patch("/jokes/:id", (req, res) => {
  let id = parseInt(req.params.id);
  let editedJoke = jokes.find((joke) => joke.id === id);
  if (req.body.type) editedJoke.jokeType = req.body.type;
  if (req.body.text) editedJoke.jokeText = req.body.text;
...
app.delete("/jokes/:id", (req, res) => {
  let id = parseInt(req.params.id);
  let jokeIndex = jokes.findIndex((joke) => joke.id === id);
  let deletedJoke = jokes.splice(jokeIndex, 1);

  if (deletedJoke) res.sendStatus(200);
...
app.delete("/all", (req, res) => {
  ...
  if (token === masterKey) jokes = [];

  if (jokes.length === 0) res.sendStatus(200);
  else res.sendStatus(500);
});
